...
class Transient {
	...
	public function set_name( string $name ): void {
		...
		if ( strlen( $name ) > 172 ) {
			...
			$log->add_log( sprintf( __( 'The name %1$s for the transient exceeds limits by WordPress.', 'personio-integration-light' ), esc_html( $name ) ), 'error', 'system' );
		}
		$this->name = $name;
	}
	...
	public function display(): void {
		...
		if ( $this->has_message() ) {
			?>
			<div class="personio-integration-transient updated <?php echo esc_attr( $this->get_type() ); ?>" data-dismissible="<?php echo esc_attr( $this->get_name() ); ?>-<?php echo absint( $this->get_dismissible_days() ); ?>">
	...
	public function get_hide_on(): array {
		 ...
		 */
		return apply_filters( 'personio_integration_transient_hide_on', $hide_on, $instance );
	}
