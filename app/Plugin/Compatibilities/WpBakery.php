<?php
/**
 * File to handle the compatibility-check for WPBakery.
 *
 ...
 */
class WpBakery extends Compatibilities_Base {
	 ...
	 * @var string
	 */
	protected string $name = 'personio_integration_compatibility_wpbakery';
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using WPBakery - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> allows you to design the output of positions in WPBakery.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
