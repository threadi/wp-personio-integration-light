<?php
/**
 * File to handle the compatibility-check for Divi before 5.0.
 *
...
class Divi extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Divi - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> allows you to design the output of positions in Divi.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
