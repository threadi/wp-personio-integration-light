...
class GeneratePressPremium extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Generate Press Premium - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> allows you to design the output of positions in Generate Press Elements.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
