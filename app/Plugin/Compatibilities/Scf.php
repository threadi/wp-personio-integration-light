...
class Scf extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Secure Custom Fields (SCF) - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> allows you to extend the data of your positions with SCF.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
