<?php
/**
 * File to handle the compatibility-check for Salient WPBakery.
 *
 ...
 */
class Salient_WpBakery extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Salient WPBakery - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens new window)</a> allows you to design the output of positions in Salient WPBakery.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
