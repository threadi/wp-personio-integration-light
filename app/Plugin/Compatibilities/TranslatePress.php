...
class TranslatePress extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using TranslatePress for multiple languages in your website - very nice!</strong> <a href="%s" target="_blank"><i>Personio Integration Pro</i> (opens new window)</a> supports the multilingualism of open positions via TranslatePress.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
