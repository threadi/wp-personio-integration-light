<?php
/**
 * File to handle the compatibility-check for Polylang.
 *
 ...
 */
class Polylang extends Compatibilities_Base {
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Polylang for multiple languages in your website - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> supports the multilingualism of open positions via Polylang, including language-specific URLs.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
