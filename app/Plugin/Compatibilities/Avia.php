<?php
/**
 * File to handle the compatibility-check for Avia (Enfold).
 *
 ...
 */
class Avia extends Compatibilities_Base {
	 ...
	 * @var string
	 */
	protected string $name = 'personio_integration_compatibility_avia';
	...
	public function check(): void {
		...
		if ( $this->is_active() ) {
			...
			$transient_obj->set_message( sprintf( __( '<strong>We realized that you are using Enfold - very nice!</strong> <a href="%1$s" target="_blank"><i>Personio Integration Pro</i> (opens a new window)</a> allows you to design the output of positions in their own page builder Avia.', 'personio-integration-light' ), esc_url( Helper::get_pro_url() ) ) );
	...
	/**
	 * Check if Avia and its necessary plugins are active.
	 *
	 * @return bool
	 */
	/**
