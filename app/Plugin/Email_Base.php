...
class Email_Base {
	 ...
	 *
	 * @param \PersonioIntegrationLight\Dependencies\easySettingsForWordPress\Settings $settings_obj
	 * @param Tab                                                                  $email_tab
	 *
	 * @return void
	 */
	public function add_settings( \PersonioIntegrationLight\Dependencies\easySettingsForWordPress\Settings $settings_obj, Tab $email_tab ): void {
		...
		$description = __( 'Add one or more recipients for this email. One email per field. Get more fields after saving the settings.', 'wp-personio-integration' ) . ' ';
		if( ! empty( $this->get_default_recipient() ) ) {
			$description .= sprintf( __( '<strong>If no email is set we use the admin-email %1$s as recipient.</strong> You can edit the admin-email of your WordPress <a href="%2$s">here</a>.', 'wp-personio-integration' ), '<code>' . $this->get_default_recipient() . '</code>', $wp_general_settings_url );
		}
		else {
			$description .= '<strong>' . __( 'Without recipient this email will not be sent.', 'wp-personio-integration' ) . '</strong>';
		}
		...
		$field->set_title( __( 'Add recipients', 'wp-personio-integration' ) );
	...
	public function get_recipients(): array {
		...
		// if setting is empty and we are using the test mode, use the admin email.
